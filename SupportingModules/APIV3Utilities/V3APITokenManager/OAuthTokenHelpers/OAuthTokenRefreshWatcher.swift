...
protocol OAuthTokenRefreshWatchable {
    var ongoingTokenRefresh: AnyPublisher<Bool, Never>? { get }
...
class OAuthTokenRefreshWatcher: OAuthTokenRefreshWatchable {
    ...
    private var status: CurrentValueSubject<OAuthTokenStatus, Never>
    ...
    var ongoingTokenRefresh: AnyPublisher<Bool, Never>? {
    ...
    init(tokenStatusSource: CurrentValueSubject<OAuthTokenStatus, Never>? = nil) {
        status = tokenStatusSource ?? CurrentValueSubject(.unknown)
    }

    /// Sets the current token status, skipping values identical to the current one
    func updateRefreshingStatus(newValue: OAuthTokenStatus) {
        guard status.value != newValue else { return }
        status.send(newValue)
    }
}
