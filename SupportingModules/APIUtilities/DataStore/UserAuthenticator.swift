...
struct UserAuthenticator {
    ...
    fileprivate static func getAccessToken(withUsername username: String, andPassword password: String) -> Observable<Any> {
        return OAuthSender.requestAccessToken(withUsername: username, andPassword: password)
            .map { (token) -> Any in
                guard let accessToken = token.accessToken, let refreshToken = token.refreshToken, let expiresIn = token.expiresIn else { return "" }
                UserAuthenticator.saveCredentials(token: accessToken, refreshToken: refreshToken, secondsExpiresIn: expiresIn)
                return accessToken
        }
    }

    fileprivate static func getAccessToken(withCode code: String) -> Observable<Any> {
        return OAuthSender.requestAccessToken(withCode: code)
            .map { (token) -> Any in
                guard let accessToken = token.accessToken, let refreshToken = token.refreshToken, let expiresIn = token.expiresIn else { return "" }
                UserAuthenticator.saveCredentials(token: accessToken, refreshToken: refreshToken, secondsExpiresIn: expiresIn)
                return accessToken
